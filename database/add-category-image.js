 ...
 * Usage: node database/add-category-image.js
 */
...
async function migrate() {
  ...
  try {
    ...
    if (cols.length === 0) {
      await conn.execute(`ALTER TABLE categories ADD COLUMN image TEXT NULL AFTER name`);
      console.log('  ✅ Column "image" added to categories table');
    } else {
      console.log('  ⏭️  Column "image" already exists in categories table');
    }
  } catch (err) {
